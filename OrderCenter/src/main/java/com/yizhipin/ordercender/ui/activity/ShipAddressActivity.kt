...
class ShipAddressActivity : BaseMvpActivity<ShipAddressPresenter>(), ShipAddressView, View.OnClickListener {
    ...
    private fun refreshEditStatus() {
        val isEditStatus = getString(R.string.mannger) == mHeaderBar.getRightText()
//        mAddAddressView.setVisible(isEditStatus)
        mHeaderBar.getRightTv().text = if (isEditStatus) getString(R.string.complete) else getString(R.string.mannger)
    }
