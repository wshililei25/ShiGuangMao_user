...
class ShipAddressRepository @Inject constructor() {
    ...
    /*
        设置默认
     */
    fun setDefaultShipAddress(map: MutableMap<String, String>): Observable<BaseResp<Boolean>> {
        return RetrofitFactoryPost(map).create(ShipAddressApi::class.java).setDefaultShipAddress(map.get("id")!!)
    }
